...
# Initialize global variable for counting malicious files
malicious_count = 0
...
def scan_file(file_data, api_key):
    global malicious_count  # Access the global variable
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if report.get('positives', 0) > 0:
                    # Increment the count if a malicious file is detected
                    malicious_count += 1
                return report
            else:
...
def index():
    ...
    if request.method == 'POST':
        ...
        api_key = "b8be961e4b156cbfb7be6476bd4fefcc33d25cb614a61e3cbacceb00349a2fac"  # Replace with your actual VirusTotal API key

        file_id = scan_file(file, api_key)
        if file_id:
            print_slow(colorama.Fore.YELLOW + "Analyzing...\n")
            report = get_report(file_id, api_key)
            return render_template('result.html', report=report, malicious_count=malicious_count)  # Pass the malicious count to the template
        else:
